...
class Explorer(agents.Agent):
    ...
    def __init__(self, program=None, heading='east', environment=None, verbose=True):
        ...
        # default None; if environment is defined, then register_environment sets these
        self.location = None
        ...
        # default None; if environment is defined, then register_environment call to
        # reset knowledge state will set these
    ...
    def register_environment(self, environment):
        if self.verbose:
            print(f"{self.__class__.__name__}.register_environment()")
    ...
    def reset(self):
        ...
        if hasattr(self, 'initial_location'):
            ...
            # self.belief_location is location the agent believes it is in
            self.belief_location = self.initial_location
        else:
            print(f"{self.__class__.__name__}.reset(): agent has no initial_location;")
    ...
    def heading_str(self, heading):
        ...
        if isinstance(heading, int):
            if 0 <= heading <= 3:
                ...
                print(f"Not a valid heading int (0 <= heading <= 3), got: {heading}")
                sys.exit(0)
            ...
            if heading in headings:
                ...
                print(f"Not a valid heading str (one of {headings}), got: {heading}")
                sys.exit(0)
        else:
            print(f"Not a valid heading: {heading}")
            sys.exit(0)

    def heading_int(self, heading):
        """ Same comment in doc for heading_str applies...
    ...
    def pretty_percept_vector(self, pvec):
        ...
        percept_vector = list()  # = [ 'None' for i in range(len(pvec)) ]
        if pvec[0]:
    ...
    def raw_percepts_to_percept_vector(self, percepts):
        ...
        percept_vector: [<Stench?>, <Breeze?>, <Glitter?>, <Bump?>, <Scream?>]
        """
        percept_vector = [False]*5  # [ False for i in range(5) ]
...
def TraceAgent(agent):
    ...
    def new_program(percept):
        ...
        print(f'{agent} perceives {agent.pretty_percept_vector(percept)} and does {action}')
        return action
...
class WumpusEnvironment(agents.XYEnvironment):
    ...
    def __init__(self, width=4, height=4, entrance=(1, 1)):
        ...
        self.time_step = 0
        self.done = False
        self.global_percept_events = list()
    ...
    def exogenous_change(self):
        ...
        for agent in self.agents:
            ...
            if any(colocated_wumpi):
                print(f'A Wumpus ate {agent}!')
                agent.performance_measure -= 1000
                self.done = True
            elif colocated_pit:
                print(f'{agent} fell into a bottomless pit!')
                agent.performance_measure -= 1000
                self.done = True
    ...
    def percept(self, agent):
        ...
        for thing in self.things_near(agent.location):
            ...
            if isinstance(thing, tuple) and len(thing) == 2:
                perceivable_thing = thing[0]

                if isinstance(perceivable_thing, Gold):
                    # need to ensure that only show glitter IF on same square as Gold.
                    if agent.location == perceivable_thing.location:
                        percepts.append('Gold')
                else:
                    percepts.append(self.thing_percept(perceivable_thing, agent))

            else:
                print(f'WARNING WumpusEnvironment.percept(): Received unexpected thing: {thing}')
    ...
    def execute_action(self, agent, action):
        ...
        if action == 'TurnRight':
            ...
            self.move_to(agent, utils.vector_add(self.heading_to_vector(agent.heading),
                                                 agent.location))
        elif action == 'Grab':
            if self.some_things_at(agent.location, tclass=Gold):
                try:
                    ...
                    print(f'All things: {self.list_things_at(agent.location)}')
                    print(f'Gold?: {self.list_things_at(agent.location, tclass=Gold)}')
                    sys.exit(-1)
    ...
    def shoot_arrow(self, agent):
        ...
        aloc = agent.location
        while True:
            aloc = utils.vector_add(dvec, aloc)
            if self.some_things_at(aloc, tclass=Wumpus):
                try:
                    ...
                    print(f'All things: {aloc} {self.list_things_at(aloc)}')
                    print(f'Wumpus?: {aloc} {self.list_things_at(aloc, tclass=Wumpus)}')
                    sys.exit(-1)
