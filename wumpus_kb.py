...
def action_forward_str(t=None):
    ...
    return 'Forward{0}'.format(t) if t is not None else 'Forward'


def action_grab_str(t=None):
    """Action Grab executed at time <t>"""
    return 'Grab{0}'.format(t) if t is not None else 'Grab'


def action_shoot_str(t=None):
    """Action Shoot executed at time <t>"""
    return 'Shoot{0}'.format(t) if t is not None else 'Shoot'


def action_climb_str(t=None):
    """Action Climb executed at time <t>"""
    return 'Climb{0}'.format(t) if t is not None else 'Climb'


def action_turn_left_str(t=None):
    """Action Turn Left executed at time <t>"""
    return 'TurnLeft{0}'.format(t) if t is not None else 'TurnLeft'


def action_turn_right_str(t=None):
    """Action Turn Right executed at time <t>"""
    return 'TurnRight{0}'.format(t) if t is not None else 'TurnRight'


def action_wait_str(t=None):
    """Action Wait executed at time <t>"""
    return 'Wait{0}'.format(t) if t is not None else 'Wait'


def add_time_stamp(prop, t):
    return '{0}{1}'.format(prop, t)
...
def generate_square_OK_axioms(t, xmin, xmax, ymin, ymax):
    ...
    return [s for s in axioms if s != '']  # filter(lambda s: s != '', axioms)
...
def generate_non_location_ssa(t):
    ...
    """
    axioms = list()  # all_state_loc_ssa(t, xmin, xmax, ymin, ymax)
