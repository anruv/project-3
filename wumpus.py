...
class WumpusWorldScenario(object):
    ...
    def __init__(self, layout_file=None, agent=None, objects=None,
        ...
        """
        if agent is not None and not isinstance(agent, wumpus_environment.Explorer):
    ...
    def build_world(self, width, height, entrance, agent, objects):
        ...
        """
        env = wumpus_environment.WumpusEnvironment(width, height, entrance)
        if self.trace:
        ...
        for row in layout:
            ...
            if row:
                ...
                for cell in row:
                    ...
                    for char in cell:
                        if char == 'W':
                            objects.append((wumpus_environment.Wumpus(), (ci, ri)))
                        elif char == 'P':
                            objects.append((wumpus_environment.Pit(), (ci, ri)))
                        elif char == 'G':
                            objects.append((wumpus_environment.Gold(), (ci, ri)))
                        elif char == 'A':
                            entrance = (ci, ri)
        ...
        for step in range(steps):
            if self.env.is_done():
                ...
                for agent in self.env.agents:
                    slist.append(f' {agent}={agent.performance_measure}')
                    if agent.verbose:
                        if hasattr(agent, 'number_of_clauses_over_epochs'):
                            print(f"number_of_clauses_over_epochs: {agent.number_of_clauses_over_epochs}")
                        if hasattr(agent, 'belief_loc_query_times'):
                            print(f"belief_loc_query_times: {agent.belief_loc_query_times}")
                print(''.join(slist))
                return
            self.step()

    def to_string(self):
        s = f"Environment width={self.width}, height={self.height}\n"
        s += f"Initial Position: {self.entrance}\n"
        s += f"Actions: {self.actions}\n"  # TODO: Why does a Scenario have actions???
        return s
...
def world_scenario_hybrid_wumpus_agent_from_layout(layout_filename):
    ...
    return WumpusWorldScenario(layout_file=layout_filename,
                               agent=wumpus_agent.HybridWumpusAgent('north', verbose=True),
                               trace=False)
...
def wscenario_4x4_HybridWumpusAgent():
    return WumpusWorldScenario(agent=wumpus_agent.HybridWumpusAgent('north', verbose=True),
                               objects=((wumpus_environment.Wumpus(), (1, 3)),
                                        (wumpus_environment.Pit(), (3, 3)),
                                        (wumpus_environment.Pit(), (3, 1)),
                                        (wumpus_environment.Gold(), (2, 3))),
...
def world_scenario_manual_with_kb_from_layout(layout_filename):
    ...
    """
    return WumpusWorldScenario(layout_file = layout_filename,
                               agent=with_manual_kb_program(wumpus_agent.HybridWumpusAgent('north', verbose=True)),
                               trace=False)
...
def wscenario_4x4_manual_HybridWumpusAgent():
    return WumpusWorldScenario(agent=with_manual_kb_program(wumpus_agent.HybridWumpusAgent('north', verbose=True)),
                               objects=((wumpus_environment.Wumpus(), (1, 3)),
                                        (wumpus_environment.Pit(), (3, 3)),
                                        (wumpus_environment.Pit(), (3, 1)),
                                        (wumpus_environment.Gold(), (2, 3))),
...
def world_scenario_manual_from_layout(layout_filename):
    ...
    """
    return WumpusWorldScenario(layout_file=layout_filename,
                               agent=with_manual_program(wumpus_environment.Explorer(heading='north', verbose=True)),
                               trace=False)
...
def wscenario_4x4_manual():
    return WumpusWorldScenario(agent=with_manual_program(wumpus_environment.Explorer(heading='north', verbose=True)),
                               objects=((wumpus_environment.Wumpus(), (1, 3)),
                                        (wumpus_environment.Pit(), (3, 3)),
                                        (wumpus_environment.Pit(), (3, 1)),
                                        (wumpus_environment.Gold(), (2, 3))),
...
def with_manual_kb_program(agent):
    ...
    def show_propositions():
        ...
        print("     '" + '\', \''.join(wumpus_kb.proposition_bases_atemporal_location) + '\'')
        print("   Perceptual propositions (include time index: P<t>)")
        print("     '" + '\', \''.join(wumpus_kb.proposition_bases_perceptual_fluents) + '\'')
        print("   Location fluent propositions (include x,y and time index: P<x>_<y>_<t>)")
        print("     '" + '\', \''.join(wumpus_kb.proposition_bases_location_fluents) + '\'')
        print("   State fluent propositions (include time index: P<t>)")
        print("     '" + '\', \''.join(wumpus_kb.proposition_bases_state_fluents[:4]) + '\',')
        print("     '" + '\', \''.join(wumpus_kb.proposition_bases_state_fluents[4:]) + '\'')
        print("   Action propositions (include time index: P<t>)")
        print("     '" + '\', \''.join(wumpus_kb.proposition_bases_actions) + '\'')
    ...
    def check_kb_status():
        ...
        """
        result = wumpus_agent.minisat(agent.kb.clauses)
        if result:
    ...
    def location_based_query(proposition_base):
        ...
        """
        display_env = wumpus_environment.WumpusEnvironment(agent.width, agent.height)
        start_time = perf_counter()  # clock()
        print(f"Running queries for: {proposition_base}<x>_<y>")
        for x in range(1, agent.width+1):
            for y in range(1, agent.height+1):
                query = logic.expr(f'{proposition_base}{x}_{y}')
                result = agent.kb.ask(query)
                if result is None:
                    display_env.add_thing(wumpus_agent.Proposition(query, '?'), (x, y))
                else:
                    display_env.add_thing(wumpus_agent.Proposition(query, result), (x, y))
        end_time = perf_counter()  # clock()
        print(f"          >>> time elapsed while making queries: {end_time-start_time}")
        print(display_env.to_string(agent.time,
                                    title=f"All {proposition_base}<x>_<y> queries"))
    ...
    def location_time_based_query(proposition_base, time):
        ...
        display_env = wumpus_environment.WumpusEnvironment(agent.width, agent.height)
        start_time = perf_counter()  # clock()
        print(f"Running queries for: {proposition_base}<x>_<y>_{time}")
        for x in range(1, agent.width+1):
            for y in range(1, agent.height+1):
                query = logic.expr(f'{proposition_base}{x}_{y}_{time}')
                result = agent.kb.ask(query)
                if result is None:
                    display_env.add_thing(wumpus_agent.Proposition(query, '?'), (x, y))
                else:
        ...
        print(f"          >>> time elapsed while making queries: {end_time-start_time}")
        print(display_env.to_string(agent.time,
                                    title=f"All {proposition_base}<x>_<y>_{time} queries"))

    def run_all_queries(time):
        check_kb_status()
        for p in wumpus_kb.proposition_bases_perceptual_fluents:
            simple_query(p + '{0}'.format(time))
        for p in wumpus_kb.proposition_bases_atemporal_location:
            location_based_query(p)
        for p in wumpus_kb.proposition_bases_location_fluents:
            location_time_based_query(p,time)
        for p in wumpus_kb.proposition_bases_state_fluents:
            simple_query(p + '{0}'.format(time))
        ...
        if time-1 > 0:
            ...
            for p in wumpus_kb.proposition_bases_actions:
                simple_query(p + f'{time-1}')
    ...
    def manual_kb_program(percept):
        ...
        action = None
        while not action:
            print(f"[{agent.time}] You perceive: {agent.pretty_percept_vector(percept)}")
            val = input("Enter Action ('?' for list of commands): ")
            val = val.strip()
            if val in helping:
                print()
                ...
                if len(q) == 2 and q[0] == 'qp':
                    ...
                    print(f"'{val}' is an invalid command; try again (enter '?' for list of commands)\n")

        # update KB with selected action
        agent.kb.tell(wumpus_kb.add_time_stamp(action, agent.time))
...
def run_minisat_test():
    ...
    for query, expected_result in queries:
        ...
        result = wumpus_agent.minisat(query, None, variable=None, value=True, verbose=False)
        print(f"  Query CNF:  {query}")
        print(f"  Result:     {result.success}   (Expected: {expected_result})")
        if result.success != expected_result:
            print("    FAILURE: unexpected result.")
            failed.append(t)
        if result.success:
            print(f"  Variable Assignment: {result.varmap}")
        t += 1
    print("-----------------------------------------------------")
    if not failed:
        print(f"Successfully passed {len(queries)} tests.")
    else:
        print(f"Passed {len(queries) - len(failed)} test(s).")
        print(f"The following tests failed: {failed}")
    print("DONE.")
